...
dhangadi.on('connection', (socket) => {
    ...
    socket.on('waypoints', (waypoints) => {
        Client.find({missionRequest: true, socketName:'dhangadi'}).exec()
            .then((data) => {
                for (let id in data) {
    ...
    socket.on('getMission', (data) => {
        ...
        Client.updateOne({clientId:socket.id, socketName: 'dhangadi'},{$set:{missionRequest : true}}).exec()
        .then((updated)=> {
    ...
    socket.on('disconnect', () => {
        ...
        Client.findOne({clientId:socket.id,socketName:'dhangadi'}).exec()
        .then((data) => {
            Client.deleteOne({clientId :socket.id, socketName: 'dhangadi'}).exec()
                .catch((err) => {
            ...
            if (data.deviceName === 'website') {
                ...
                Count.find({}).exec()
                    .then((data) => {
                        ...
                        Count.updateOne({},{$set:{dhangadiNo: fileNo+1}}).exec()
                            .catch((err) => {
